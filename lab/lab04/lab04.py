...
def my_reduce(combiner, seq):
    ...
    if len(seq) == 1:
        return seq[0]
    pre = combiner(seq[0],seq[1])
    for i in range(2,len(seq)):
        pre = combiner(pre,seq[i])
    return pre
...
def count_palindromes(L):
    ...
    2
    """
    return functools.reduce(lambda x,y:x+y,map(lambda x: 1 and x.lower() == x[::-1].lower(), filter(lambda s:s.lower() == s[::-1].lower(),L)))
