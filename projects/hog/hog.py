...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    if num_rolls == 0:
        total_score += boar_brawl(player_score,opponent_score)
    else:
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for player_score in range(0,goal):
        for opponent_score in range(0,goal):
            if prev == -1:
                ans = prev = strategy(player_score,opponent_score)
            else:
                prev = strategy(player_score,opponent_score)
                if ans != prev:
                    return False
    return True        
...
2.0    called SAMPLES_COUNT times.
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    maxnum, index = averaged_dice(1,dice),1
    for i in range(2, 11):
        cur_num = averaged_dice(i,dice)
        if cur_num > maxnum:
            maxnum = cur_num
            index = i
    return index
