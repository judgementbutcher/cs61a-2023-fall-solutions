...
def balanced(m):
    ...
    "*** YOUR CODE HERE ***"
    # 尝试不进行分类讨论，在内部有不同处理
    torque_of_left, torque_of_right = 0, 0
    left_mobile, right_mobile = True,True
    if is_planet(end(left(m))):
        torque_of_left = length(left(m)) * mass(end(left(m)))
    else:
        torque_of_left = length(left(m)) * total_mass(end(left(m)))
        left_mobile = balanced(end(left(m)))
    if is_planet(end(right(m))):
        torque_of_right = length(right(m)) * mass(end(right(m)))
    else:
        torque_of_right = length(right(m)) * total_mass(end(right(m)))
        right_mobile = balanced(end(right(m)))
    if torque_of_left == torque_of_right and left_mobile == True and right_mobile == True:
        return True
    else:
        return False
    # 并且还要求其悬挂的所有mobile都是平衡的
    ...
    if is_leaf(t):
        ...
        for branch in branches(t):
            if not is_leaf(branch):
                max_path = max(max_path, label(t) + max_path_sum(branch)) 
            else:
                max_path = max(max_path, label(t) + label(branch))
        return max_path
