...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        cnt = 0
        tmp = self.place
        while tmp.is_hive == False:
            if random_bee(tmp.bees) == None or (cnt < self.lower_bound or cnt > self.upper_bound):
                tmp = tmp.entrance
            else:
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        ...
        original_damage -= self.insects_shot * 0.0625
        cdamage = original_damage - distance * 0.25
        if cdamage > 0:
            return cdamage
        else:
            return 0
